...
import com.nabd.app.api.*
import com.nabd.app.data.ClaudeChatImporter
import com.nabd.app.data.GptChatImporter
import com.nabd.app.data.ApiKeyEntry
import com.nabd.app.data.ConversationSettings
import com.nabd.app.data.CustomProviderConfig
import com.nabd.app.data.DataExporter
import com.nabd.app.data.DataImporter
import com.nabd.app.data.EmbeddingIndexer
import com.nabd.app.data.EmbeddingModelConfig
import com.nabd.app.data.EmbeddingModelType
import com.nabd.app.data.LocalChatModelConfig
...
import com.nabd.app.data.local.ChatEntity
import com.nabd.app.data.local.EmbeddingEntity
...
import com.nabd.app.model.ChatConversation
import com.nabd.app.util.Constants
import com.nabd.app.util.SearchResultFormatter
import com.nabd.app.util.SnackbarEvent
...
import com.nabd.app.model.SelectedAttachment
...
class ChatViewModel(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            if (settingsManager.autoUpdateCheck.first()) {
                ...
                if (now - lastCheck > 24 * 60 * 60 * 1000L) {
                    ...
                    val info = com.nabd.app.util.UpdateChecker.check(getCurrentVersion())
                    if (info != null) {
    ...
    private val _updateDialogData = MutableStateFlow<com.nabd.app.util.UpdateInfo?>(null)
    val updateDialogData: StateFlow<com.nabd.app.util.UpdateInfo?> = _updateDialogData.asStateFlow()
    ...
    suspend fun checkForUpdates(): com.nabd.app.util.UpdateInfo? {
        val current = getCurrentVersion()
        return com.nabd.app.util.UpdateChecker.check(current)
    }
    ...
    suspend fun semanticSearch(query: String, limit: Int = 20): List<Pair<MessageEntity, Float>> {
        val ctx = com.nabd.app.viewmodel.GenerationContext(
    ...
    private fun buildGenerationPair(
        ...
        val genCtx = com.nabd.app.viewmodel.GenerationContext(
    ...
    fun addShellDevice(device: com.nabd.app.data.ShellDeviceConfig) {
        viewModelScope.launch {
    ...
    fun updateShellDevice(device: com.nabd.app.data.ShellDeviceConfig) {
        viewModelScope.launch {
            val current = shellDevices.value.toMutableList()
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            for (msg in allMsgs) {
                ...
                if (msg.attachmentMeta != null) {
                    try {
                        val meta = kotlinx.serialization.json.Json.decodeFromString<com.nabd.app.model.AttachmentMeta>(msg.attachmentMeta!!)
    ...
    fun stopGeneration() {
        ...
        NabdForegroundService.stop(getApplication())
    }
    ...
    fun sendMessage(text: String, images: List<String> = emptyList(), attachments: List<SelectedAttachment> = emptyList()): Boolean {
        ...
        generationJob = generationScope.launch {
            ...
            val metaItems = mutableListOf<com.nabd.app.model.AttachmentItem>()
            ...
            for (att in attachments) {
                when (att.type) {
                    "image" -> {
                        ...
                        metaItems.add(com.nabd.app.model.AttachmentItem(
                    ...
                    "pdf" -> {
                        val pagePaths = if (att.preRenderedPaths != null && att.preRenderedPaths.isNotEmpty()) {
                            ...
                            com.nabd.app.util.PdfPageRenderer.renderAsImages(app, android.net.Uri.parse(att.uri), att.selectedPages)
                        }
            ...
            val attachmentMeta = if (adjustedMetaItems.isNotEmpty()) {
                com.nabd.app.model.AttachmentMeta(items = adjustedMetaItems)
            } else null
    ...
    fun importClaudeChat(uri: Uri, strategy: com.nabd.app.ui.settings.ImportStrategy, selectedIds: Set<String>) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (strategy == com.nabd.app.ui.settings.ImportStrategy.REPLACE) {
    ...
    fun importGptChat(uri: Uri, strategy: com.nabd.app.ui.settings.ImportStrategy, selectedIds: Set<String>) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
